          ...
          return (
            <el-select attrs={item.attrs} v-model={this.form[item.key]}>
              {item.options.map(opt => {
                return <el-option key={opt.value} label={opt.label} value={opt.value}></el-option>
              })}
            </el-select>
          )
        case 'datetime':
...
/*
先定义表单的结构，定义一个数组，数据里在放入数组
这个数组的个数代表一共分几行
数组里的一个对象代表列，可以定义表单中item的label和type
在这个封装的表单组件中，接收这个数组，在render函数中使用jsx渲染数据结构
定义一个渲染row的函数，在函数里遍历数组里的数组作为行，在这个数组里在嵌套一个渲染列的函数，
在渲染列的函数中，遍历传递过来的参数，作为列渲染，在这个函数里又嵌套一个渲染具体的表单根据type，如input、select等
*/